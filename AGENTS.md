...
- `pnpm test` - Full test suite (lint + prettier + typescript + unit tests with coverage)
- `pnpm run test-unit` - Run node:test unit tests only
- `pnpm run lint` - ESLint validation
- `pnpm run check-prettier` - Prettier formatting check
- `pnpm run compile-typescript` - TypeScript compilation check
